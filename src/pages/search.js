...
const SearchPage = ({ data, location }) => {
  ...
  const miscPublications = data.misc.nodes.map(node => ({
    ...
    date: node.date || '',
    rawDate: parseDate(node.date),
    slug: node.attach ? node.attach.publicURL : `/publications/${node.slug || ''}`,
    content: '', // No content for misc
    excerpt: ''
  }));

  // Combine all content and sort by date (newest first)
  const allContent = [...blogPosts, ...publications, ...miscPublications]
  ...
  return (
    <Layout>
      <section className="container py-5">
        ...
        {query && (
          <p className="mb-4">
            Found {filteredContent.length} result{filteredContent.length !== 1 ? "s" : ""} for "
            <strong>{query}</strong>"
          </p>
        )}
        ...
        {filteredContent.length > 0 ? (
          <div className="search-results text-start">
            {filteredContent.map((item) => {
              ...
              const fullContent = [item.title, item.subtitle, item.abstract, item.content, item.excerpt]
                .filter(Boolean).join(" ");
              ...
              return (
                <article key={item.id} className="search-result mb-3 p-3 bg-light rounded">
                  <div className="d-flex justify-content-between align-items-start">
                    <div>
                      <h3 className="h4 mb-2 d-inline">
                        <Link to={item.slug} className="text-decoration-none ">
                          <span dangerouslySetInnerHTML={{ __html: highlightText(item.title, query) }} />
                        </Link>
                      </h3>
                    </div>
